...
int smallestValue(BTNode *node)
{
	if (node == NULL) return 987654321;
    int le = smallestValue(node->left);
    int ri = smallestValue(node->right);
    if (le >= ri && node->item >= ri) return ri;
    else if(ri >= le && node->item >= le) return le;
